...
class SendEventToGoogleWithUniversalAnalytics
{
     ...
     * payments updating from "pending" to "completed" statuses. When it does, then we check the date of the donation and if
     ...
     * @unreleased
     */
    public function __invoke(int $donation_id, string $new_status, string $old_status)
    {
        if (!$this->settingRepository->canSendEvent(TrackingMode::UNIVERSAL_ANALYTICS)) {
            return false;
        }
